...
async def _sense_zero_tail(conn, limit: int) -> List[int]:
    """Bulk sense tokens with zero price/mcap for N consecutive seconds."""
    window = int(getattr(config, 'ZERO_TAIL_CONSEC_SEC', 20))
    if window <= 0: return []
    
    rows = await conn.fetch("""
        WITH last_metrics AS (
            SELECT token_id, usd_price, mcap,
                   row_number() OVER (PARTITION BY token_id ORDER BY ts DESC) as rn
            FROM token_metrics_seconds
            WHERE token_id IN (SELECT id FROM tokens WHERE (wallet_id IS NULL OR wallet_id = 0))
        ),
        dead_tokens AS (
            SELECT token_id
            FROM last_metrics
            ...
            GROUP BY token_id
            HAVING COUNT(*) >= $2
               AND COUNT(*) FILTER (WHERE COALESCE(usd_price, 0) > 0 OR COALESCE(mcap, 0) > 0) = 0
        )
        SELECT token_id FROM dead_tokens LIMIT $1
    """, limit, window)
    return [int(r["token_id"]) for r in rows]


async def _sense_frozen_price(conn, limit: int) -> List[int]:
    """Bulk sense tokens with frozen price for N consecutive seconds."""
    window = int(getattr(config, 'FROZEN_PRICE_CONSEC_SEC', 0))
    if window <= 0: return []
    eps = float(getattr(config, 'FROZEN_PRICE_EQUAL_EPS', 1e-10) or 0.0)
    
    rows = await conn.fetch("""
        WITH last_metrics AS (
            SELECT token_id, usd_price,
                   row_number() OVER (PARTITION BY token_id ORDER BY ts DESC) as rn
            FROM token_metrics_seconds
            WHERE usd_price IS NOT NULL AND usd_price > 0
              AND token_id IN (SELECT id FROM tokens WHERE (wallet_id IS NULL OR wallet_id = 0))
        ),
        frozen_tokens AS (
            SELECT token_id
            FROM last_metrics
            ...
            GROUP BY token_id
            HAVING COUNT(*) >= $1
               AND (MAX(usd_price) - MIN(usd_price)) <= $3
        )
        SELECT token_id FROM frozen_tokens LIMIT $1
    """, limit, window, eps)
    return [int(r["token_id"]) for r in rows]
    ...
    for tid in ids:
        ...
        await conn.execute(
            ...
            SET cleaner_flagged = TRUE,
                cleaner_flag_reason = COALESCE(cleaner_flag_reason, $2),
                cleaner_flag_iteration = COALESCE(cleaner_flag_iteration, $3),
                cleaner_flagged_at = COALESCE(cleaner_flagged_at, CURRENT_TIMESTAMP)
            WHERE id = $1
            """,
            tid,
            reason,
...
async def run_cleanup(dry_run: bool = True, older_than_sec: int = 15, limit: int = 200,
    ...
    async with pool.acquire() as conn:
        ...
        try:
            ...
            # 1. Sensing Phase (Find dead/frozen tokens in bulk and flag them)
            if not dry_run:
                zt_ids = await _sense_zero_tail(conn, limit)
                if zt_ids:
                    await _flag_tokens(conn, zt_ids, "zero_tail")
                
                frz_ids = await _sense_frozen_price(conn, limit)
                if frz_ids:
                    await _flag_tokens(conn, frz_ids, "frozen_price")
            ...
            low_holders_candidates = await _find_flagged_tokens(conn, "low_holders", remaining)
            low_holders_candidates = await _filter_tokens_without_wallet(conn, low_holders_candidates)
            if low_holders_candidates:
                if dry_run:
                    ids.extend(low_holders_candidates)
                else:
                    archived, removed = await _process_flagged_tokens(conn, low_holders_candidates, "low_holders", archive_threshold)
                    if archived:
                        archived_summary["low_holders"] = archived_summary.get("low_holders", 0) + archived
                    if removed:
                        bad_removed["low_holders"] = bad_removed.get("low_holders", 0) + removed
                remaining = max(0, remaining - len(low_holders_candidates))

            no_price_candidates = await _find_flagged_tokens(conn, "no_price", remaining)
            no_price_candidates = await _filter_tokens_without_wallet(conn, no_price_candidates)
            if no_price_candidates:
                if dry_run:
                    ids.extend(no_price_candidates)
                else:
                    archived, removed = await _process_flagged_tokens(conn, no_price_candidates, "no_price", archive_threshold)
                    if archived:
                        archived_summary["no_price"] = archived_summary.get("no_price", 0) + archived
                    if removed:
                        bad_removed["no_price"] = bad_removed.get("no_price", 0) + removed
                remaining = max(0, remaining - len(no_price_candidates))
