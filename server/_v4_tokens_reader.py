...
class TokensReaderV4:
    ...
    def __init__(self, debug: bool = False):
        ...
        self.last_tokens_json: str = "" # Cash for the last sent JSON
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                for row in rows:
                    ...
                    profit_usd = None # Removed from DB
    ...
    async def _auto_refresh_loop(self):
        while True:
            try:
                ...
                # Throttle: send full list not more often than every 2.0s 
                # UNLESS the token count changed (which is a major event)
                count_changed = (count != self.last_token_count) or (total_count != self.total_token_count)
                content_changed = (last_updated != self.last_updated_at_sum)
                
                # We update if count changed OR (content changed and enough time passed)
                should_update = count_changed or (content_changed and time_since_last >= 2.0)
                    ...
                    if result["success"] and result["tokens"]:
                        ...
                        for client in self.connected_clients:
                            try:
                                ...
                                self.last_broadcast_time = now # Update only on successful send
                            except Exception as e:
