...
class BalanceV4:
    ...
    async def _get_multiple_sol_balances(self, addresses: List[str]) -> List[Optional[float]]:
        """Get multiple SOL balances in a single batch RPC call.
        Returns a list of balances corresponding to the input addresses.
        Each element is float (SOL) or None if that specific account/query failed.
        """
        if not addresses:
            return []
        try:
            # Solana permits up to 100 accounts in a single getMultipleAccounts call.
            # We process in chunks of 100 just in case.
            all_balances = []
                ...
                async with self.session.post(self.rpc_url, json=payload) as response:
                    if response.status == 200:
                        ...
                        if "result" in data and "value" in data["result"]:
                            for account_info in data["result"]["value"]:
                                if account_info and "lamports" in account_info:
                                    all_balances.append(account_info["lamports"] / 1_000_000_000)
                                else:
                                    # Account might not exist or be empty
                                    all_balances.append(0.0)
                        else:
                            all_balances.extend([None] * len(chunk))
                    else:
                        all_balances.extend([None] * len(chunk))
            return all_balances
        except Exception as e:
            print(f"[_get_multiple_sol_balances] Batch RPC Error: {e}")
    ...
    async def get_sol_balances_for_wallets(self, wallets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # Use batch RPC call instead of parallel single calls
        balances = await self._get_multiple_sol_balances(addresses)
        
        wallet_balances = []
        for i, wallet in enumerate(wallets):
            sol_balance = balances[i]
    ...
    async def _wallets_refresh(self, keys_wallets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        async with pool.acquire() as conn:
            ...
            # Step 1: Get all SOL balances in a single batch RPC call
            addresses = [w.get('address', '') for w in keys_wallets if w.get('address')]
            valid_wallets = [w for w in keys_wallets if w.get('address')]
            
            balances = await self._get_multiple_sol_balances(addresses)
            
            balance_results = []
            for i, sol_balance in enumerate(balances):
                w = valid_wallets[i]
                balance_results.append((sol_balance, int(w.get('id')), w.get('name') or f"Wallet {int(w.get('id'))}"))
