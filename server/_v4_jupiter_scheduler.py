...
from _v4_analyzer_jupiter import get_analyzer
from _v4_new_tokens import get_scanner
...
class JupiterScheduler:
    ...
    def __init__(self) -> None:
        ...
        self._min_gap: float = float(getattr(config, 'JUPITER_MIN_INTERVAL_SEC', 0.3))
        self._jitter_min: float = float(getattr(config, 'JUPITER_JITTER_MIN_SEC', 0.05))
        self._jitter_max: float = float(getattr(config, 'JUPITER_JITTER_MAX_SEC', 0.10))
            ...
            if isinstance(data, dict) and data.get("error"):
                if str(data.get("error")).strip().endswith("429"):
                    ...
                    if isinstance(retry_after, (int, float)) and retry_after > 0:
                        # User requested strict 1.5s pause, but we respect server if it asks for MORE.
                        # However, user said "pause 1.5s and go". Let's enforce 1.5s minimal.
                        self._backoff_until = max(self._backoff_until, now + 1.5)
                    else:
                        self._backoff_until = max(self._backoff_until, now + 1.5)
                return {"success": False, "error": data.get("error"), "tokens": len(tokens)}

            updated = await analyzer.save_batch_data(data)
    ...
    async def _scanner_tick(self) -> Dict[str, Any]:
        ...
        if not res or not res.get("success"):
            ...
            if isinstance(err, str) and err.strip().endswith("429"):
                # User requested strict 1.5s pause for 429
                self._backoff_until = max(self._backoff_until, asyncio.get_running_loop().time() + 1.5)
    ...
    async def _loop(self) -> None:
        ...
        while self.is_running:
            ...
            if on_scanner_tick and not self._skip_scanner:
                try:
                    res = await self._scanner_tick()
                    if res and not res.get("success"):
                        print(f"[Scheduler] Scanner tick failed: {res.get('error')}")
                except Exception as e:
                    print(f"[Scheduler] Scanner tick exception: {e}")
            else:
                try:
                    res = await self._analyzer_tick()
                    if res and not res.get("success"):
                        print(f"[Scheduler] Analyzer tick failed: {res.get('error')}")
                except Exception as e:
                    print(f"[Scheduler] Analyzer tick exception: {e}")
            ...
            await asyncio.sleep(max(0.01, float(getattr(config, 'JUPITER_ANALYZER_INTERVAL', 0.5))))
    ...
    async def _cleaner_loop(self) -> None:
        ...
        from _v4_cleaner import run_cleanup
        while self.is_running:
            try:
                await run_cleanup(
                    dry_run=False,
    ...
    async def start(self) -> Dict[str, Any]:
        ...
        return {"success": True, "message": "Scheduler V4 started"}
